 ...
 **/
  ...
  */
  public func decodeLength(v: Uint8Array): Nat8
    ...
    return  matchr Hex.decode(Hex.encode(v.toArray())), val[0], return D.trap("not a valid hex")
  ...
  public func encodeLength(len: Nat, offset: Nat): Uint8Array
    ...
    let result = matchr Hex.decode(firstByte # hexLength), val, return D.trap("not a valid hex")
  ...
  **/
  ...
  public func stripHexPrefix(str: Text): Text 
    return if (isHexPrefixed(str))
        take Text.stripStart(str, #text("0x")), str
    ...
    switch(v)
      ...
      case(#string(item))
        if (isHexPrefixed(item))
          return toBuffer<Nat8>(matchr Hex.decode(padToEven(stripHexPrefix(item))), val, D.trap("nat a valid hex"))
        return toBuffer<Nat8>(Blob.toArray(Text.encodeUtf8(item)))
      case(#number(v))
        return matchr Hex.decode(Hex.encodeByte(Nat8.fromNat(v))), toBuffer<Nat8>(val), D.trap("not a valid hex")
      case(#bigint(v))
        return matchr Hex.decode(Hex.encodeByte(Nat8.fromNat(v))), toBuffer<Nat8>(val), D.trap("not a valid hex")
      case(#Null)
